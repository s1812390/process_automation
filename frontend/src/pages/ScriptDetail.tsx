...
import { Play, Trash2, Plus, Mail, MessageCircle, X, ChevronLeft } from 'lucide-react'
...
type Tab = 'editor' | 'requirements' | 'settings' | 'alerts' | 'history'
...
export default function ScriptDetail() {
  ...
  // Initialize form from script data
  useState(() => {
    if (script && !settings) {
      setEditorContent(script.script_content)
      setReqContent(script.requirements_content || '')
      setSettings({
  ...
  const handleSaveEditor = () => {
    updateMutation.mutate({ script_content: editorContent })
  }

  const handleSaveReq = () => {
    updateMutation.mutate({ requirements_content: reqContent })
  }
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => {
              if (confirm(`Delete "${script.name}"?`)) deleteMutation.mutate()
            }}
      ...
      {activeTab === 'editor' && (
        <div className="space-y-4">
          <ScriptEditor
            value={editorContent || script.script_content}
      ...
      {activeTab === 'requirements' && (
        <div className="space-y-4">
          <p className="text-[12px] text-ink-3">
            Enter pip packages, one per line or in standard requirements.txt format.
          </p>
          <ScriptEditor
        ...
        <div className="bg-white rounded-lg border border-[rgba(99,112,156,0.12)] p-6 space-y-5">
          <div>
            ...
            <CronInput
              ...
              onChange={(v) => setSettings({ ...settings, cron_expression: v })}
            />
          </div>
            ...
            <div className="flex items-center gap-3 pt-5">
              ...
              <button
                ...
                className={clsx(
                  ...
                  settings.is_active ? 'bg-success' : 'bg-[rgba(99,112,156,0.2)]'
                )}
              >
                <span
                  className={clsx(
      ...
      {activeTab === 'alerts' && (
        <div className="space-y-4">
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className="bg-white rounded-lg border border-[rgba(99,112,156,0.12)] p-4 flex items-center justify-between"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[11px] text-ink-3 mt-0.5">
                    {[alert.on_failure && 'failure', alert.on_success && 'success', alert.on_timeout && 'timeout']
                      .filter(Boolean)
                      .join(', ')}
                  </div>
                </div>
          ...
          {newAlert ? (
            <div className="bg-white rounded-lg border border-[rgba(99,112,156,0.12)] p-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewAlert({ ...newAlert, channel: e.target.value as 'email' | 'telegram' })}
                    className="w-full px-3 py-2 rounded-lg border border-[rgba(99,112,156,0.2)] bg-white text-[13px] text-ink-1 focus:outline-none focus:border-violet"
                  >
                ...
                {(['on_failure', 'on_success', 'on_timeout'] as const).map((key) => (
                  <label key={key} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setNewAlert({ ...newAlert, [key]: e.target.checked })}
                      className="w-3.5 h-3.5 accent-violet"
                    />
                    <span className="text-[12px] text-ink-2 font-[600]">
                      {key.replace('on_', 'On ')}
                    </span>
                  </label>
                ))}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg text-[13px] font-[700] bg-white text-ink-2 border border-[rgba(99,112,156,0.2)] hover:bg-bg active:scale-[0.97] transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={() => createAlertMutation.mutate(newAlert)}
                  disabled={!newAlert.destination || createAlertMutation.isPending}
            ...
            <button
              onClick={() =>
                setNewAlert({ channel: 'email', destination: '', on_failure: true, on_success: false, on_timeout: true })
              }
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[rgba(240,242,247,0.6)]">
                ...
                <th className="text-left px-4 py-2.5 text-[10.5px] font-[700] uppercase tracking-[0.9px] text-ink-3">Attempt</th>
              ...
              {history?.items.map((run) => (
                <tr key={run.id} className="border-t border-[rgba(99,112,156,0.06)] hover:bg-accent/[0.025]">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link to={`/runs/${run.id}`} className="text-[11px] font-[700] text-violet hover:text-violet/70">
                      Logs
                    </Link>
                  </td>
                </tr>
