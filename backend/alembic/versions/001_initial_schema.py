...
def _table_exists(conn, name: str) -> bool:
    result = conn.execute(
        text("SELECT COUNT(*) FROM user_tables WHERE table_name = :n"),
        {"n": name.upper()},
    )
...
def upgrade() -> None:
    ...
    # SH_APP_SETTINGS
    if not _table_exists(conn, "SH_APP_SETTINGS"):
        op.create_table(
            "SH_APP_SETTINGS",
    ...
    # SH_SCRIPTS
    if not _table_exists(conn, "SH_SCRIPTS"):
        op.create_table(
            "SH_SCRIPTS",
    ...
    # SH_SCRIPT_RUNS
    if not _table_exists(conn, "SH_SCRIPT_RUNS"):
        op.create_table(
            "SH_SCRIPT_RUNS",
            sa.Column("id", sa.Integer, sa.Identity(always=True), primary_key=True),
    ...
    # SH_RUN_LOGS
    if not _table_exists(conn, "SH_RUN_LOGS"):
        op.create_table(
            "SH_RUN_LOGS",
            sa.Column("id", sa.Integer, sa.Identity(always=True), primary_key=True),
    ...
    # SH_ALERT_CONFIGS
    if not _table_exists(conn, "SH_ALERT_CONFIGS"):
        op.create_table(
            "SH_ALERT_CONFIGS",
            sa.Column("id", sa.Integer, sa.Identity(always=True), primary_key=True),
